...
public class AppsTab extends SuperTabView {

	private static final Logger logger = LogManager.getLogger(AppsTab.class);
	
	public AppsTab(EventPortalView view, Composite parent, int index, RefreshListener tabbedView) {
		super(view, parent, SupportedObjectType.APPLICATION, index, tabbedView, true);
	}
...
//				iconType = AnimType.APP;
				...
				TreeNode appNode = builder.build("App", name, details, null, null, time);
...
//					item.setImage(imageRegistry.get("vApp"));
					...
					name = vName(appVer.getVersion(), app.getName());
...
//					TreeNode appVerNode =  new TreeNode(type, appVer.getId(), appVer, iconType, "vApp", name, details, state, topic, updated, "View AsyncAPI");
					...
					for (String eventId : both) {
						helperAddEventVersion(eventId, appVerNode, Dir.BOTH, brokerType, TSF);
					}
					for (String eventId : pubs) {
						helperAddEventVersion(eventId, appVerNode, Dir.PUB, brokerType, TSF);
					}
					for (String eventId : subs) {
						helperAddEventVersion(eventId, appVerNode, Dir.SUB, brokerType, TSF);
					}
				}
			}
		}
	...
	static void helperAddEventVersion(String vEventId, TreeNode nodeParent, Dir pubSub, String brokerType, TimeStringFormat TSF) {
		...
		ApplicationVersion appVer = (ApplicationVersion)nodeParent.epObject;
		Application app = (Application)nodeParent.getParent().epObject;
...
//			TreeNode eventVerNode =  new TreeNode(SupportedObjectType.EVENT_VERSION, vEventId, eventVer, iconType, "vEvent", pubSub.arrows + " ID: " + vEventId, pubSub.big + " " + brokerType + " Event", null, null, null, "View in Portal");
			...
			if (!origDomainId.equals(app.getApplicationDomainId())) builder.withOrigDomainId(origDomainId);
			String name = String.format("%sv%s %s%s",
					origDomainId.equals(app.getApplicationDomainId()) ? "" : "(EXT) ",
					eventVer.getVersion(),
					event.getName(),
					event.getShared() ? "*" : "");
